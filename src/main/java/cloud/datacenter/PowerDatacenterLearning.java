...
import cloud.policy.VmAllocationAssignerLearning;
...
import static cloud.ExecuteData.LearningScheduleTest.brokerId;
...
public class PowerDatacenterLearning extends PowerDatacenter {
    ...
    private double cloudletSubmitted; // The final time when submit the cloudlet.
    private int migrationCount; // VM migration count
    private double currentTime; // Current time
    private final VmAllocationAssignerLearning vmAllocationAssignerLearning; // Policy based on the reinforcement learning
    private Host targetHost; // Current VM target host
    private String currentcpu; // Current CPU status
    private String historycpu; // History CPU status
    public static List<List<Double>> everyhosthistorypower = new ArrayList<>(); //The history power consumption of the hostlist
    public static List<Double> allpower = new ArrayList<>(); // Total power consumption
    ...
    public PowerDatacenterLearning(
            ...
            double schedulingInterval, VmAllocationAssignerLearning vmAllocationAssignerLearning) throws Exception {
    ...
    public String convertCPUUtilization(List<Double> cpuutils) {
        ...
        for (Double cpuutil : cpuutils) {
            ...
            if (cpuutil >= 0 && cpuutil < 10) {
                ...
                System.out.println("Errors occur while converting cpuutil " + cpuutil);
                break;
            }
    ...
    protected void processVmCreate(SimEvent ev, boolean ack) {
        Vm vm = (Vm) ev.getData();
        int hostid = vmAllocationAssignerLearning.createAction(currentcpu); //Select the allocated host for vm
