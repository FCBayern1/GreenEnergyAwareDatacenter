...
import cloud.*;
import cloud.datacenter.PowerDatacenterLearning;
...
import static cloud.policy.VmAllocationAssignerLearning.QList;
...
//            printCloudletList(newList);
        ...
        for (String s : QList.keySet()) {
            System.out.println(s + ":" + QList.get(s)+" QList key");
        }
...
//        GenExcel.getInstance().genExcel();
        for (int i = 0; i < PowerDatacenterLearning.allpower.size(); i++) {
            System.out.println(PowerDatacenterLearning.allpower.get(i)+" "+i+" th VM power");
