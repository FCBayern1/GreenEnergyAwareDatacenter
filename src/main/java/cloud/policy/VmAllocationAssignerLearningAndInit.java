...
public class VmAllocationAssignerLearningAndInit { //强化学习分配策略
    ...
    public void initRowOfQList(String state_idx, double reward, Vm vm) { //初始化Q值表的行
    ...
    public int createAction(String cpulist, Vm vm) { //生成选择虚拟机的动作，即获得想要的虚拟机号
        ...
        if (!QList.containsKey(state_idx)) { //若Q值表中不存在这一行，则初始化这一行
            initRowOfQList(state_idx, finalreward, vm);
        }
        //根据随机数x选择生成动作的方式
    ...
    public void updateQList(int action_idx, double reward, String lastcpulist, String cpulist, Vm vm) { //更新Q值表
        finalreward = reward;
        ...
        if (!QList.containsKey(next_state_idx)) { //若更新后的行不存在于Q值表中，则初始化它
            initRowOfQList(next_state_idx, finalreward, vm);
        }
